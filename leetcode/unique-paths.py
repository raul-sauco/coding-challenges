...
# What the problem asks for is simply the number of ways we can combine
# a certain amount of 2 values,
# for example, if we call the down move 0 and the right move 1, the
# first example can be re-written to be:
...
# rate, even limited by a size of n, m <= 100,
# the factorial of 200 is > 1e350 which is probably more space than
# needed for the 100 slot list used on the DP.
#
# The results running this code in LeetCode are a bit surprising, for me
...
# We can use functools.reduce to have python do the calculations in C,
# this method has the same O complexity but it is more efficient.
#
# Time complexity: O(m*n) - we do one calculation for each position of
# the matrix mxn.
...
class Reduce:
    def uniquePaths(self, m: int, n: int) -> int:
        if 1 in [n, m]:
            return 1
        return reduce(lambda x, y: x * y, range(n, n + m - 1)) // reduce(
            lambda x, y: x * y, range(1, m)
        )
